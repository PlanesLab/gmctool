...
server <- function(input, output, session) {
  ...
  observeEvent(c(input$I_perc_GEA_class_1, input$I_perc_GEA_class_2,
                 ...
                 input$I_mp4_calculate_force,
                 input$I_class_1_30_filter_targeted_class,
                 input$I_class_1_30_unfilter_targeted_class),{
  ...
  # if the table is changed (single / double), don't show image and reset selected row
  observeEvent(input$I_RESULT_TABLE_GMCS_SINGLE_DOUBLE, {
    values$O_table_essential_gmcs_rows_selected_previous_selection = 0
    flags$flag_show_heatmap_gmcs = 0
  })
  ...
  filter_DepMap_tables <- function(){
    ...
    values$DepMap.info.filtered$DepMapEssentialityByGene <- readRDS(paste0("./Data/DepMap_Data/DepMapEssentialityByGene_",
      ...
      filter(ENSEMBL_target %in% gMCS.info$selected$genes.gMCSs.ENSEMBL)
    ...
    # values$DepMap.info.filtered$DepMapEssentialityByGene %>% head()
    # values$DepMap.info.filtered$DepMapExpressionByGene %>% head()
    # values$DepMap.info.filtered$DepMapCorrelationByGene %>% head()
    # values$DepMap.info.filtered$DepMapGeneExpression %>% head()
  }
  ...
  ResultsTableCorrFiltersGene <- reactive({
    ...
    if (flags$flag_show_table_gmcs_correlation){
      ...
      if (input$I_DepMap_database_filter_mode != "none" & length(setdiff(input$I_DepMap_database_filter_selected, '---'))>0) {
        ...
        aux <- merge(na.omit(aux2) %>% select("DepMap_ID", "essentiality_score","ENSEMBL_target"),
                     na.omit(aux3) %>% select("DepMap_ID", "logTPM",  "ENSEMBL_target")) 
        aux <- split(aux, as.character(aux$ENSEMBL_target))
  ...
  observeEvent(c(flags$flag_show_dotplot_gmcs,
                 ...
                 input$I_show_separate_biomarker,
                 input$O_table_essential_gmcs_bis_rows_selected), {
                   ...
                   if (flags$flag_show_dotplot_gmcs){
                     ...
                     values$plot.obj.DepMap <- ShowDotplot_DepMap(DepMap.info.all = values$DepMap.info.filtered,
                                                                  ...
                                                                  flag_color_by_partner_gene = input$I_show_separate_biomarker,
